...
inline void GetAverageSignals(float* Average, bool GetError, const float* beamintensity, const float apertures[], const int num_apertures, TTree* tree, const float* beamintensity_branch, const float* collaperture_branch, const float* signal_branch, const int* voltage_branch, const int* MC);
...
int main(int const argc, char const * const * const argv) {
  ...
  Detector->SetBranchStatus("IsMC",1);
  ...
  Detector->SetBranchAddress("IsMC", &ismc);
  ...
  for(int intensity_iterator = 0; intensity_iterator < recorded_beamIntensities.size(); ++intensity_iterator){
    ...
    GetAverageSignals(SignalAverage, false, &recorded_beamIntensities.at(intensity_iterator), Aperture, n, Detector, &beamintensity, &collaperture, &signal, &voltage, &ismc);
    float SignalAverageError[n];
    GetAverageSignals(SignalAverageError, true, &recorded_beamIntensities.at(intensity_iterator), Aperture, n, Detector, &beamintensity, &collaperture, &signal, &voltage, &ismc);
  ...
  legend->Draw();
  for(int i = 0; i < All_TGraphErrors.size(); ++i){
    All_TGraphErrors.at(i)->Write();
  }
  outputfile->Write();
  canvas->Print( (outputfilename+"Canvas.cxx").c_str() );
  //outputfile->Close();

  //inputfile->Close();
}  

inline void GetAverageSignals(float* SignalAverage, bool GetError, const float* beamintensity, const float apertures[], const int num_apertures, TTree* tree, const float* beamintensity_branch, const float* collaperture_branch, const float* signal_branch, const int* voltage_branch, const int* MC){
  ...
  for(int number_apertures = 0; number_apertures < num_apertures; ++number_apertures){
    TH1D* temp = new TH1D("signal-noise", title1D.c_str(),1000,0,120000);
    Signal_CollAperture.push_back(temp);
  }
    ...
    if(*voltage_branch > 0){
      if(*MC == 0){
        ...
        for(int number_apertures = 0; number_apertures < num_apertures; ++number_apertures){
          //Fill the TH1 in the vector with signals for an aperture, that corresponds to the desired apertures in the aperture vector:
          if(*collaperture_branch > apertures[number_apertures]-0.4 && *collaperture_branch < apertures[number_apertures]+0.4){
            Signal_CollAperture.at(number_apertures)->Fill( (*signal_branch / *beamintensity_branch)*4836 );//Scale the MC signal output to the real data #entries
          }
        }
      }
    }
