...
int main(int const argc, char const * const * const argv){
  UsePhDStyle();
	...
	if (argc < 3) {
		...
		std::cerr << "./CompareHitTime -i file1.root file2.root -o output" << std::endl;
		exit(2);
	}
  ...
  Plot_everything( inputfilenames, "HitTime_All", outputfilename);
...
void Plot_everything(std::vector< std::string > inputfilenames, std::string histo_name, std::string outputfilename) {
  ...
  legend->Draw();
  canvas->Update();
  canvas->Print( ("output/HitTime_Comparison_"+outputfilename+".pdf").c_str() );
  canvas->Print( ("output/HitTime_Comparison_"+outputfilename+".cxx").c_str() );
}
