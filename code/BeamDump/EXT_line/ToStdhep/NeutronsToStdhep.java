...
public class NeutronsToStdhep {
	...
	private static void Usage() {
		...
		System.out.println("\n For example: \n"
			+ ">> java -cp bin:lib/* MuonsToStdhep -i muons1.txt muons2.txt -o muons.stdhep -r 2 -n 3000");
		System.exit(0);
	}//end Usage()
...
class Particle{
	...
	public Particle(double[] qualities){
		...
		cos = new double[3];
		cos[0] = qualities[3];//directional cosine between x direction and z-axis
		cos[1] = qualities[4];//directional cosine between y direction and z-axis
		cos[2] = qualities[5];//directional cosine between z direction and z-axis
		mom_tot = qualities[6];
		time = -qualities[7]; //time difference between muons and beam (negative sign because negative time in text file means later than beam)
		weight = qualities[8];
