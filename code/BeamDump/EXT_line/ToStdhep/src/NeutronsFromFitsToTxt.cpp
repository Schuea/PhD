...
void * createEvent(void * threadID)
{
...
int main()
{
  ...
  int events = 5909704;//Update me!
  if(events % NUM_THREADS != 0) events += events % NUM_THREADS;
  double x_X(0);
  double y_X(0);
  double x_Y(0);
  double y_Y(0);
  ...
  bool set_T(false);
  bool fit_failed(false);
  pfile = fopen("Neutrons_leaving_EXT_1bunch.txt","w");
  ...
  for(int i = 0; i < events; ++i)
  {
    ...
    if(i != 0 && i % NUM_THREADS == 0)
    {
      ...
      {
        pthread_join(threadID[j],&status);
      }
    }
    pthread_create(&threadID[i % NUM_THREADS],NULL,createEvent,NULL);
  }
  for(int j = 0; j < NUM_THREADS; ++j)
  {
    pthread_join(threadID[j],&status);
