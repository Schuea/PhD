...
An algorithm was written \todo{was or has been written?} that calculates the helix tracks of the pair particles using their four-vectors. 
...
Up to \SI{6.2}{\centi\meter} from the IP, where the beam pipe increases in width, the beam pipe radius is \SI{1.2}{\centi\meter}. \todo{English}
...
The additional electric circuits that are required to meet the higher demand, and with it the supplementary cooling, lead in the end to a larger material budget, which is not desirable for the vertex detector.
...
The number of the dead cells is calculated from the occupancy plots in Figures (a) and (c), and depends on the buffer depth.
...
& 2 & & & &\\
& 3 & & & &\\
\multirow{-5}{*}{\rotatebox[origin=c]{90}{\parbox[c]{1.9cm}{\cellcolor{white}\centering \sid vertex endcap}}} & 4 & & & &\\
\hline\hline
\end{tabularx}
\end{table}
...
For the clarification of the impact on the pair background occupancy, the following section looks at the hit distribution in the \sid detector.
 ...
 \centering
   ...
   \caption[Pair background occupancy in the \sid vertex detector for the ILC250 set (A) for different \sid geometry variants]{Pair background occupancy for the ILC250 parameter set (A) in the innermost \sid vertex detector barrel layer, after a full bunch train (\num{1312} bunch crossings).
...
For the \sid detector, this leads to the positive implication that the vertex detector readout architecture can be designed to be suitable for both ILC stages regarding the pair background.\\
\begin{figure}
 \centering
  \begin{subfigure}[b]{0.49\textwidth}
   \centering
...
The luminosity of this stage is doubled with respect to the ILC500 stage by doubling the number of bunches per train to \num{2625}.
...
The number of beam bunches has therefore also a direct impact on the pair background and the \sid occupancy.
  \begin{figure}
 \centering
  \begin{subfigure}[b]{0.49\textwidth}
   \centering
