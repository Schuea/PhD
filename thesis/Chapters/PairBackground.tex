...
The arising detector occupancy is, however, dependent on several factors, such as the ILC accelerator configurations.
...
The longitudinal momentum of the \positron\electron pairs reaches roughly \SI{80}{\percent} of the beam energy, whilst the transverse momentum extends to only \SI{0.8}{\GeV} for the ILC250, and \SI{1.6}{\GeV} for the ILC500.
 ...
 \end{figure}
\\Due to their low transverse momentum, the pairs are deflected on narrow \todo{narrow the right word?} helical tracks in the magnetic field of the detector solenoid magnet.
...
\\As the bell shaped envelope is fully symmetric in the positive and negative z-directions, as well as in the xz and yz-plane, the view of the pair background envelopes is chosen to be in the xz-plane in the positive z-direction. \todo{show xy plane}
 \begin{figure}
 \centering
  \begin{subfigure}[b]{0.49\textwidth}
   \centering
   ...
   \end{subfigure}
   \caption[Pair background density]{Pair background density in the xz-plane for one ILC bunch crossing.
 ...
 \end{figure}
\\In order to compare the envelope shapes for different ILC beam parameters, Figure~\ref{fig:PairBkg:Density} (b) shows the envelopes containing a certain fraction of all tracks.
...
The pair background simulation files for this comparison were generated with \guineapig, using the beam parameters of the three different baseline ILC stages~\cite[p. 11]{TDR1}. \todo{show density plots for 250 and 350GeV}
...
Nevertheless, there are non-zero \todo{English} particle densities observed outside the beam pipe, extending beyond the vertex detector layers.
These densities, however, are below 5 particles per x-bin (\todo{how big is 1 bin?}).
...
The ratio for set (A) is still below the critical limit with about \num{4.2e-5}.\\
 ...
 \centering
  \begin{subfigure}[b]{0.49\textwidth}
   ...
   \caption{\sid vertex detector \textbf{barrel},\\ratio of dead cells}
   \end{subfigure}\\
  \begin{subfigure}[b]{0.49\textwidth}
   \centering
   ...
   \caption{\sid vertex detector \textbf{endcaps},\\ratio of dead cells}
   \end{subfigure}
...
The number of dead cells for a buffer depth of four, shown in Figure~\ref{fig:PairBkg:ILC250_Occupancy_Layer0} (b), is now at the critical limit of \num{e-4} of all cells.
...
\\In the ECAL endcaps, only up to three hits per cell are observed because of which the ratio of dead cells are calculated up to a buffer depth of three, as can be seen in Figure~\ref{fig:PairBkg:ILC250_Occupancy_Inner_detectors} (c).
Similarly in the HCAL endcaps (Figure~\ref{fig:PairBkg:ILC250_Occupancy_Inner_detectors} (d)), the ratio of dead cells is calculated for an assumed buffer depth of up to two only.
The pair background particles do not affect the calorimeter endcap performance.
\begin{figure}
 \centering
   \begin{subfigure}[b]{0.49\textwidth}
   \centering
...
In further \sid subdetectors that are populated by the pair background, the ratio of dead cells for a buffer depth of four does not reach the acceptance limit, which was shown in Section~\ref{PairBkg:further_subdetectors}.
...
For the clarification of the impact on the pair background occupancy, the following section looks at the hit distribution in the \sid detector.
 ...
 \centering
   \begin{subfigure}[b]{0.49\textwidth}
   \centering
    \includegraphics[width=\textwidth]{Figures/Pairs/Occupancy_Comparison_Layer_0_numcells_ILC250_SetA_All_SiD_designs_SiVertexBarrel_corrected_Barrel_size.png}
       \caption{\sid vertex detector \textbf{barrel}}
   \end{subfigure}
   \hfill
    \begin{subfigure}[b]{0.49\textwidth}
   \centering
    \includegraphics[width=\textwidth]{Figures/Pairs/Occupancy_Comparison_Layer_0_numcells_ILC250_Comparison_Set_A_All_SiD_designs_SiVertexEndcap.png}
...
The number of dead cells for a given buffer depth is doubled in both subdetectors for the ``Lumi Up'' stage, exceeding the critical acceptance limit of \num{e-4}.
The number of beam bunches therefore also has a direct impact on the pair background and the \sid occupancy.
In order to stay below a critical occupancy limit, the ILC upgrade to the ILC500 ``Lumi Up'' stage seems to be a suitable time for an upgrade of the \sid sensor technology as well.
\begin{figure}
 \centering
  \begin{subfigure}[b]{0.49\textwidth}
   \centering
