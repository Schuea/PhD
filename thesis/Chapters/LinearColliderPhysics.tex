...
\label{fig:RFPhase}
\end{figure}
\\However, in linear accelerators, bunching is done differently, in several stages.
The initial bunching is done directly after the particle source, the very first stage of acceleration.
As the particles are at this stage still non-relativistic, one of the possible methods that can be used is ``velocity bunching''.
...
Particles arriving at the cavity at a phase with negative voltage will be decelerated, whilst particles arriving at a phase with positive voltage will be accelerated respectively (see Figure~\ref{fig:Velocity_bunching}).
...
Again, there are several methods, which all in principle are based on the idea of shortening the bunch by exploiting the momentum spread of the beam particles.
