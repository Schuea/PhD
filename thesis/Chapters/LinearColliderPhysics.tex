...
\todo{Does this last sentence sound odd?}
\end{chapterabstract}
\newline
...
By this \todo{or: ``Following this''?} logic a particle is accelerated to higher energies by applying higher and higher electrostatic fields. 
...
The need for higher order correction magnets can also be derived from the expansion of the magnetic field around the ideal path of the particle (x = 0):\todo{Check the English in the few sentences above!}
\begin{alignat}{5}
...
A schematic cross-section of a quadrupole magnet is shown in Figure~\ref{fig:Quadrupole}, in which a positively charged particle would be focused in the y-direction and defocused in the x-direction.\todo{Fig. 3.3b):``Into the page'' correct?} 
...
FODO is an alternating structure made out of focusing quadrupole magnets (``F'') and defocusing ones (``D''), with drift paths (``O'') in between (see Figure~\ref{fig:FODO}). \todo{Fig. 3.4: Better word for compressed here?}
...
Of particular interest is its value at the interaction point (IP), the point of the beam collisions, where it is then called $\beta^*$.\todo{How is beta star defined exactly?}
...
The normalized emittance, $\epsilon\gamma$, (with $\gamma$ being here the relativistic Lorentz factor) is the quantity which is adiabatically invariant under energy change, and is commonly given as one of the typical beam parameters of a particle accelerator.
\begin{figure}[h]
\centering
...
\\
With this knowledge, the overall beam size can be calculated by using the distribution spread of the particle momentum for all particles:
\begin{align}
...
For relativistic particles traveling at the speed of light, their orbit along the accelerator however depends on the difference of their momentum $\Delta p$ to the ideal momentum $p$.
...
The synchronous phase is therefore chosen to be off the acceleration-peak of the wave.
The particles oscillate about thid ideal stable phase, and therefore form bunches.
...
Although being a circular collider, it was therefore possible to collide leptons and to do important precision measurements of the W and Z boson.\todo{Re-word that sentence}
