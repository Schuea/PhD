...
\todo{Collider: Add Rutherford reference}
...
A solution was quickly found by using time dependent electric fields, which change the orientation of their field periodically.\\
In simple terms, the key principles of a linear collider are thereby already explained.
\todo{Collider: This sentence is somehow weird and unnecessary, consider removing or rewording it}
...
\todo{Collider: Is the electric resistance not zero?}
...
\todo{Collider: I don't think you explained what an RF frequency is, although you are quoting one on this line}
\begin{figure}
...
Circular acceleration has certain challenges.
...
To insure the stability of the orbit, not only can bending dipole magnets be found, but also quadrupole magnets for focusing the particles, and sextupole and octupole magnets which then correct orbital fluctuations.
...
\end{alignat}
Equation~\ref{eq:field_components} shows the connection between the radius and the magnetic field in the dipole component (compare with Equation~\ref{eq:MagField_Radius}).
...
The smaller the beta function, the narrower the beam.\\
...
\todo{Collider: You mean whilst all having momentum that are very similar to each other, not exactly the same}
...
\todo{Collider: Is the above sentence the explanation of the one above it? If so remove the word 'Also'}
...
Every second about 780 million events are occurring at the LHC, because of which it is a so-called ``discovery machine''.
\todo{Collider: Is it really to do with the rate of events, that the LHC is a discover machine? I thought it was the higher CoM energy}
...
One advantage of circular accelerators was already mentioned, namely the possibility to go to high energies by simply letting the beams perform more revolutions and hence gain more and more energy from every turn.
...
The only way to increase the collision energy of a linear collider is to make the accelerating line longer.
...
Every deflection in a magnetic field yields an energy loss in the form of this synchrotron light, and the more magnets an accelerator has, the more energy loss the beam suffers from.
\todo{Collider: What if we have no dipole magnets but lots of quadrupole and sextupole magnets?}
...
\todo{Collider: Does the ratio of the energy loss depend on the radius? If not, why include the radius in this sentence?}
\todo{Collider: I think the radius of the LHC is a little larger than that}
\begin{align*}
