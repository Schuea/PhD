...
One of the first sub-systems is the inital bunching and pre-acceleration of the beam with normal conducting RF cavities.
This is done by the method of ``velocity bunching'', as explained in Section~\ref{AccPhysics:Bunching}.
Being pre-accelerated to only \SI{76}{\MeV} in this step, the now bunched beams are accelerated to \SI{5}{\GeV} by a  superconducting booster linac~\cite[p. 81f]{TDR32}.
Since the electrons and positrons have still a large emittance from when they originated from their sources, their momentum and beam size have to be condensed.
\todo{Check the English of the sentences above}
...
In both compressor systems, RF cavities are used together with undulators, which compact the beam momentum by exploiting the momentum spread within the beam bunch.
As discussed in Section~\ref{AccPhysics:Bunching}, the bunch tail experiences an accelerating phase in the RF cavity, whilst the bunch head experiences a decelerating phase.
The low-momentum particles of the bunch head will therefore be deflected more in the undulator, the high-momentum particles of the tail are deflected less, respectively.
At the end of the compressor system, the particles of the bunch tail and head are gotten closer, leading to a shortened beam bunch. 
\todo{Check the English of the sentences above}
