...
In order to confirm theories in particle physics, or to disproof them, it is often important to be able to measure the qualities of particles or their interactions to the tenth decimal place or better.
  \todo{ILC: I know this is a pain, but can you find an example of when we need to know something to the tenth decimal place and add a reference?}
For measurements being done in high-energy particle colliders, such precisions can only be reached in linear lepton colliders, as has already been shown in Chapter~\ref{LinearColliderPhysics}.
  \todo{ILC: I'm not sure I saw something in the linear collider chapter that showed me the actual precision achieved by linear colliders}
...
\todo{ILC: Do I understand correctly, that you actually produce more positrons than electrons?}
\begin{figure}
\centering
...
\end{figure}
\\
The polarized positrons then continue along the positron line that is, from that point on, equivalent to the electron line.
...
This is desired in the damping rings in order to ``cool'' the beams, i.e. to reduce their momentum due to the emission of light.
...
\todo{ILC: Add a reference to XFEL}
...
\todo{ILC: Related to the todo in the LinearCollider chapter, if the luminosity determines the discovery ability of the machine, then surely we should already have the ILC instead of the LHC?}
...
\todo{ILC: I think you are missing a word or two in this sentence}
...
The total costs for realizing the ILC includes next to the manpower not only the costs for the construction of the so-called conventional facilities, such as the underground tunnels and the surface buildings, but also the production and assembling of the accelerator and detector components.
\todo{ILC: This needs rewording, ask me}
When breaking the total costs down into the subsystems, then the conventional facilities together with the production of the RF systems for the accelerator make up almost 75\,\% of the total costs.~\cite[p. 20f]{TDR1}
...
These options would yield a cost reduction of up to 34\,\%, resulting in about 66\,\% of the initial costs for the \SI{500}{\GeV} machine by reducing the length of the tunnel (options A, B and C), or by additionally reducing the number of RF cavities (options A', B' and C').~\cite{Cost_reduction}
...
\todo{ILC: The above sentence needs rewording}
...
However, the other parts of the accelerator and the surface buildings will not change by the staging scenarios but form basic costs because of which the maximum cost reduction that can be achieved is only 34\,\%.
Therefore, it has to be considered whether the cost cutback from option A' outweighs the positive sign a longer tunnel would give to the ILC community that an energy upgrade is foreseen for the future.\\
\begin{figure}[h]
\centering
...
\label{fig:ILC_Site}
\end{figure}
\todo{ILC: Figure caption: Is it still just a possible site?}
...
\todo{ILC: Above sentence needs rewording}
...
\todo{ILC: Explain why the weak interactions are enhanced or suppressed by beam polarization}
...
Because the elementary coupling of Z bosons and photons is the same for all quarks and leptons, the \electron \positron annihilation produces pairs of all species, from the Standard Model and Beyond Standard Model, at similar rates.
...
\todo{ILC: What events are there that don't couple to electroweak interactions?}
PDF uncertainties and systematic uncertainties due to QCD corrections are omitted.
\todo{ILC: This is a big deal, as it is one of the major source of uncertainty for the LHC (I think)}
...
\todo{ILC: What kind of theoretical assumptions do we have to make at the LHC?}
The quark and lepton momenta can therefore be determined by kinematic fits.
\todo{ILC: I don't understand what the above sentence means}
...
\todo{ILC: Is this different to b tagging? If so how? How can we know if the small distance secondary vertex belongs to a b quark or a c quark?}
...
In contrast, the LHC experiments are not able to access the global decay width and the Higgs couplings in a model independent way.
...
\todo{ILC: The above 'equations' for WW and ZZ fusion don't seem to include WW and ZZ boson production}
...
\todo{ILC: Consider using the acronym package so that you don't have to worry about writing Standard Model (SM) in every chapter}
...
\todo{I think it is possible to find a new particle at lower precision too, but certain particles could only be discovered at the 1 percent level}
...
\todo{ILC: If you are going to say Coke, best say Coca Cola.}
\todo{ILC: Don't say Coca Cola, you don't want to be an advertisement for them, be really American about it and say 'soda can'}
...
\todo{ILC: Change every possible to almost all. Unless the detector goes through the beam pipe}
...
\todo{ILC: The detector is hermetic because it covers all angles in phi, and a large area in eta}
...
\todo{ILC: What do you mean 'Next to providing hermeticity?}
...
\todo{ILC: What is machine background?}
...
\todo{Is that the right reference to the TCMB?}
...
\begin{itemize}
 ...
 Before, the distance (L*) between the IP and QD0 was different for SiD and ILD, as L* is dependent on the size of the detector.
    \todo{ILC: Do you mean distance or difference?}
...
\label{fig:ILD}
\end{figure}
\todo{ILC: Check if I have to update the picture of the ILD detector}
