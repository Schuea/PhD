...
In order to confirm theories in particle physics, or to disprove them, it is often important to be able to measure the qualities of particles or their interactions to high accuracy.
...
The last lepton collider for high-energy physics experiments was the Large Electron-Positron Collider (LEP), which was running from 1989 until 2000~\cite{LEP}, as mentioned in Section~\ref{AccPhysics:Linear-Circular}. 
...
\\The International Linear Collider (ILC) was hence proposed, which intends to find new particles and new physics in highest precision measurements.
...
\label{ILC:layout}
To meet the physics goals discussed in the section above, the ILC is proposed to be a \positron\electron collider.
By choosing it to be a linear collider, it is extendable.
With an initial center-of-mass energy of \SI{250}{\GeV} for its first stage, the ILC can be upgraded to 350-\SI{500}{\GeV} and possibly even \SI{1}{\TeV}.

It is envisioned to have a high luminosity and state-of-the-art technologies for the detectors and the machine, which will make the ILC an unprecedented precision machine.
The designs of the collider and the detector concepts, which will be explained in the following sections, are a result of over twenty years of research at over 300 science laboratories and universities all over the world~\cite{TDR1}.
