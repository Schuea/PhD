...
With all other quantities fixed, only $R$ can be chosen to be high in order to reduce the required voltage.
...
However, the other parts of the accelerator and the surface buildings will not change by the staging scenarios but form basic costs because of which the maximum cost reduction that can be achieved lies at only 34\,\%.
...
The focus will lie on SiD, since all the background studies presented in this thesis were done in the context of the SiD detector only.
...
Latter is very challenging due to the short flight distance of the Charm quark of only a few microns before it hadronizes.
...
The changes regarding the overall accelerator and facility design are suggested as Change Requests (CR), and are reviewed and approved by a Technical Change and Management Board.~\cite{TCMB} \todo{Is that the right reference to the TCMB?}
