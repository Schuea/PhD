...
\\For a lower beam energy of \SI{125}{\GeV} (\SI{250}{\GeV} center-of-mass energy), the number of muons that are produced is lower, because of which only ?? muons reach the detector without shielding. \todo{Number of muons w/o shielding}
...
\label{BDS_Muons:SiD}
Since the SiD detector reads out the hits after every bunch train (1312 bunch crossings) only, the detector occupancy for the muons has to be studied for a muon rate per train.
...
For details on these detector characteristics, please refer to Chapter~\ref{ILC:SiD}.
...
In the material of the wall, the muons loose their energy, and low energy muons are stopped completely. \todo{Check out Bethe-Bloch here}
...
Since none of the cells are hit by more than three muons, the occupancy in the HCAL barrel is very low and far from the critical limit.
 ...
 \end{figure}
\\For the SiD tracker endcaps, however, the number of hits per cell reaches a maximum 30 (see Figure~\ref{fig:BDS_Muons:SiTrackerEndcap}).
...
Although in the next ILC stage at a center-of-mass energy of \SI{500}{\GeV}, the sheer number of muons reaching the detector is larger by at least a factor of three, also here the occupancy does not exceed \num{e-7}.
