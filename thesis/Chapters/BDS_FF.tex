...
Which qualities do these produced background particles have? \todo{Edit questions}
...
Particle accelerator lattices, like the ATF2 lattice, can easily be built up by the pre-defined components. 
...
\subsubsection{Modelling the vertical beam halo collimator}
...
At the location of the beam halo collimator, the beam core has a size of $\sigma^{Core}_x$ = \SI{0.16}{\milli\meter} and $\sigma^{Core}_y$ = \SI{0.28}{\milli\meter}.
\\Several \bdsim simulations runs were done for different apertures of the vertical beam halo collimator.
...
In the direct proximity of the collimator, the number of secondary particles is larger in the case of a closed collimator, because of particle showers caused by the intersection of the beam halo with the collimator jaws.
Further downstream of the collimator, however, the number of secondaries is smaller in every beam line component.
\\At the location of the RHUL Cherenkov detector, which is indicated in Figure~\ref{fig:ParticlesPerModel} (b), the number of occurring secondary particles is lower by over one order of magnitude.
...
The respective components are all located downstream of the collimator and upstream of the Cherenkov detector.
There is no significant difference in the number of particles between the two studied collimator apertures.
\todo{``retrieved'' the right word in Fig description?}
\begin{figure}
\centering
\begin{subfigure}[b]{0.49\textwidth}
 \centering
...
\caption[Number of particles at the RHUL Cherenkov detector]{Number of secondary particles arriving at the location of the RHUL Cherenkov detector in the case of an open (aperture of \SI[detect-all]{24}{\milli\meter}) and closed (aperture of \SI[detect-all]{4}{\milli\meter}) vertical beam halo collimator.}
\label{fig:RHUL_Cherenkov_Sampler}
\end{figure}

\subsection{Conclusion on the studies concerning the background levels in proximity of the vertical beam halo collimator}
...
In order to improve the model, measurements of all component apertures along the ATF2 line had to be taken and implemented.
...
So far, the implemented field maps do only describe the magnetic field around the beam axis.
...
For all measured beam intensities, the background photon level could be reduced by closing the collimator jaws more than \SI{6}{\milli\meter} half aperture, which corresponds to a full aperture of \SI{12}{\milli\meter}.
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.6\textwidth]{Figures/ATF/IP_background_Nuria_Thesis_p212IP_background_Nuria_Thesis_p212.pdf}
