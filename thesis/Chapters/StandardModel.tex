...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{Figures/Standard_Model_of_Elementary_Particles.png}
...
\\From that, one can calculate the average energy $E_{\gamma}$ of the beamstrahlung photons~\cite{Beamstrahlung_CLIC}: \todo{Check page number}
\begin{equation}
...
For the three processes, the effective total cross section for a single final-state particles can be written as~\cite{PairBkg_cross_section}:
\begin{align}
